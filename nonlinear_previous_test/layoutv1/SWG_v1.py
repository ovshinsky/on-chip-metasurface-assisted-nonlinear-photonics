...
def pbs(start_point_x, start_point_y, GRATING, grating_port_width):
    ...
    # left taper wg
    tp_start_point = wg_bus.get_start_point()
    tp_end_point = tp_start_point - (taper_wg_length, 0)
    tp_left = Taper(tp_start_point, tp_end_point, start_width=bus_wg_width, end_width=ring_width )
    tp_left.draw(cell, wg_layer)

    # right taper wg
    tp_start_point = wg_bus.get_end_point()
    tp_end_point = tp_start_point + (taper_wg_length, 0)
    tp_right = Taper(tp_start_point, tp_end_point, start_width=bus_wg_width, end_width=ring_width )
    tp_right.draw(cell, wg_layer)
    # # </editor-fold>
...
# col
for j in range(5):
    period1_initial = [386e-3, 416e-3,446e-3,476e-3,506e-3]
...
make_gdsii_file("PBS_v2-1.gds", inv_source_layer=wg_layer,inv_target_layer=wg_layer)
