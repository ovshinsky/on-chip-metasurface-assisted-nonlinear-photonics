...
plot(a,b);
legend("P_{idler}")
xlabel("Power_{pump}(mW)")
ylabel("Power_{idler}(mW)")
...
plot(c,b);
legend("P_{idler}")
xlabel("Power_{sig}(mW)")
ylabel("Power_{idler}(mW)")

function [p_pump,p_ider,p_sig] = FWM(k,m)
    ...
    u1_int(1) = sqrt(0.01)*k/50;
    u2_int(1) = sqrt(0.01)*k/50;
    us_int(1) =sqrt(0.001)*m/50;
    ui_int(1) =sqrt(0);
